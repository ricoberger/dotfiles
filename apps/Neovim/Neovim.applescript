...
--
-- The actual terminal is opened by the reusable "ghostty-new" helper, which
...
-- Absolute path to the ghostty-new helper. Automator/Finder launch the app with
-- a minimal PATH that excludes ~/.local/bin, so it has to be resolved via $HOME.
on ghosttyNew()
	return (system attribute "HOME") & "/.local/bin/ghostty-new"
end ghosttyNew
...
on run
	do shell script quoted form of ghosttyNew() & ¬
		" --working-dir " & quoted form of (system attribute "HOME") & ¬
		" --command " & quoted form of "nvim" & " --placement window"
end run
...
on open theFiles
	...
	do shell script quoted form of ghosttyNew() & ¬
		" --working-dir " & quoted form of cwd & ¬
		" --command " & quoted form of cmd & " --placement window"
end open
