-- See https://github.com/antonk52/filepaths_ls.nvim
...
local function resolve_completion_item(item)
  ...
  item.documentation =
    file_info_documentation(abs_path, stat, preview_from_prefix(data), target)
  return item
end
...
local function create_server(dispatchers, config)
  ...
  local function respond(method, params, callback, request_id)
    ...
    if method == METHODS.textDocument_completion then
      active_requests[request_id] = completion_items(
        params,
        state.settings,
        finish_request(request_id, callback)
      )
      return
    end
    ...
    if method == METHODS.completionItem_resolve then
      callback(
        nil,
        resolve_completion_item(params --[[@as lsp.CompletionItem]])
      )
      return
    end
