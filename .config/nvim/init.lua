...
GitHubSearch = function(searchtype, query)
  -- Fetch search results from GitHub using the "gh" command-line tool and
  -- handle any errors.
  local output = vim.fn.system(
    string.format(
      "gh search %ss %s --limit 100 --json author,body,number,repository,title,updatedAt,url",
      searchtype,
      query
    )
  )
...
vim.api.nvim_create_user_command("GitHubSearchPRs", function(opts)
  GitHubSearch("pr", opts.args)
end, {
  nargs = "*",
})

vim.api.nvim_create_user_command("GitHubSearchIssues", function(opts)
  GitHubSearch("issue", opts.args)
end, {
  nargs = "*",
})
...
vim.keymap.set(
  "n",
  "<leader>ghpi",
  "<cmd>GitHubSearchPRs --state open --involves ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghpc",
  "<cmd>GitHubSearchPRs --state open --author ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghpa",
  "<cmd>GitHubSearchPRs --state open --assignee ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghpm",
  "<cmd>GitHubSearchPRs --state open --mentions ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghpr",
  "<cmd>GitHubSearchPRs --state open --review-requested ricoberger<cr>"
)
...
vim.keymap.set(
  "n",
  "<leader>ghii",
  "<cmd>GitHubSearchIssues --state open --involves ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghic",
  "<cmd>GitHubSearchIssues --state open --author ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghia",
  "<cmd>GitHubSearchIssues --state open --assignee ricoberger<cr>"
)
vim.keymap.set(
  "n",
  "<leader>ghim",
  "<cmd>GitHubSearchIssues --state open --mentions ricoberger<cr>"
)
