...
export PATH=$GOROOT/bin:$GOPATH/bin:$HOME/.bin:$HOME/.krew/bin:$HOME/flutter/bin:/opt/homebrew/opt/openjdk/bin:$HOME/.istioctl/bin:$HOME/.cargo/bin:/opt/homebrew/opt/python@3.10/libexec/bin/:$PATH
...
alias gitwork='git config user.email "rico@staffbase.com" && git config user.name "ricoberger" && cat .git/config | grep -A 2 "user"'
alias ghd='gh dash'
